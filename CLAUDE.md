...
- **Replication ingress** (`replication_ingress.py`) — the host's door for a surrogate's stimulus batches; a composer mounts it with `ingress.add_routes(observer.app)`. Dedupes against per-origin high-water marks (`HighWaterMarks`), commits with `StimulusLog.append_many`, records unexplained holes as `stimulus.gap`, and triggers a coalesced `orient`.
