...
- **Decide**, where we translate context to action. Another invariant of a `CognitiveCore` is that they have a reasoning engine of some sort built in. I expect this will be an LLM in pretty much every case, but nevertheless I'm leaving the door open, at least for VLMs or even VLAMs. Decide emits a structured response with an action decision, which is interpretted and executed by....
- **Act**, where the agent actually does stuff. Even in the simplest case, when **Decide** emits a "Respond to user in Chat" decision, execution is separate. In the example of a simple chat response, **Act** receives the decision to respond to the user, and then figures out how to do that, likely with a `ChatResponseGenerator` module. **Act** is also crucially in charge of terminating the cognitive loop or triggering another one, but again it *does not* decide that for itself. If **Decide**'s decision requires another loop, **Act** triggers one, otherwise execution terminates.
