...
class ChatCognitiveCore(ModelProvider):
    ...
    def act(self, action: str):
        if action == "Generate Response":
            ...
            prompt = (
                "Given the following chat history and user message, generate a response to the user's message. "
                + "Chat history: "
                + str(chat_history)
                + " User message: "
                + str(self.loop_memory["current_message"])
            )
            ...
            self.working_memory.remember(self.loop_memory["current_message"])
