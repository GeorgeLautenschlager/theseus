...
consumes it. A plain `StimulusLog` never enters this mode — see buffer policy #33.
"""
...
class BufferPolicy:
    ...
    # Hysteresis line: eviction begins when the buffer exceeds this fraction of
    # max_bytes. Strictly inside (0, 1) — at 1.0 there is no hysteresis and the
    # buffer would rewrite itself on every append past the line; at 0.0 a single
    # eviction would empty the whole buffer.
    low_water: float = 0.8
